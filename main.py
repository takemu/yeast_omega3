...
def run_dha01(output_dir):
    gpr_model, _ = create_gpr_model(model_xml)
...
def run_dha02(ref_values, output_dir):
    gpr_model, _ = create_gpr_model(model_xml)
    solution = simulate_engineered_strain(gpr_model, ynb_medium, [substrate_rxns['Glc']], 0.4, 10, ['R_r_o003'], evo_base_constr | {'u_YGL055W': (2*ref_values['u_YGL055W'], 1000)})
    solution.to_dataframe().to_csv(f'{output_dir}/dha02_results.csv')


def run_dha03(ref_values, output_dir):
    gpr_model, _ = create_gpr_model(model_xml)
    solution = simulate_engineered_strain(gpr_model, ynb_medium, [substrate_rxns['Glc']], 0.4, 10, ['R_r_o003'], evo_base_constr | {'u_YGL055W': (2*ref_values['u_YGL055W'], 1000)} | {'u_YOR245C': (0, 0)})
    solution.to_dataframe().to_csv(f'{output_dir}/dha03_results.csv')


def run_dha04(ref_values, output_dir):
    gpr_model, _ = create_gpr_model(model_xml)
    solution = simulate_engineered_strain(gpr_model, ynb_medium, [substrate_rxns['Glc']], 0.4, 10, ['R_r_o003'], evo_base_constr | {'u_YGL055W': (2*ref_values['u_YGL055W'], 1000)} | {'u_YOR245C': (0, 0)} | {'u_YNR016C': (2*ref_values['u_YNR016C'], 1000)})
    solution.to_dataframe().to_csv(f'{output_dir}/dha04_results.csv')


def run_dha05(ref_values, output_dir):
    gpr_model, _ = create_gpr_model(model_xml)
    solution = simulate_engineered_strain(gpr_model, ynb_medium, [substrate_rxns['Glc']], 0.4, 10, ['R_r_o003'], evo_base_constr | {'u_YGL055W': (2*ref_values['u_YGL055W'], 1000)} | {'u_YOR245C': (0, 0)} | {'u_YNR016C': (2*ref_values['u_YNR016C'], 1000)} | {'u_YNR008W': (0, 0)})
